...
const Login = () => {
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    let { usermail, password } = loginName;
    if (usermail && password) {
      setloginName(initial);

      customAxios
        .post("/auth/login", { email:usermail })
        .then((responce) => {
  ...
  const handleChange = (prop: keyvalue) => {
    ...
    if (prop.key === "usermail") finalvalue.usermail = prop.value;
      ...
      <form onSubmit={handleSubmit} className="background-box">
        ...
        <div>
          ...
          <div className="margintop">
            <p>
              Or Register Using ClickHere..<Link to="/register">REGISTER</Link>
              <br />
              <Link to="/register">
